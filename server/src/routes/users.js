...
router.get("/schedule", async (req, res) => {
    try {
      const courseDates = await knex("users")
        .join('course_registration', 'users.id', 'user_id')
        .join('courses', 'courses.id', 'course_id')
        .select('users.id as user_id', 'date_start', 'date_end');

      const crewDates = await knex("users")
        .join('crews', 'users.crew_id', 'crews.id')
        .join('crew_rotations', 'crews.id', 'crew_rotations.crew_id')
        .select('users.id as user_id', 'date_start', 'date_end');

      const combined = [
        ...courseDates.map(d => ({ ...d, type: 'course' })),
        ...crewDates.map(d => ({ ...d, type: 'crew' }))
      ];

      const grouped = {};

      for (const entry of combined) {
        const { user_id, date_start, date_end, type } = entry;
        ...
        grouped[user_id].dates.push({ date_start, date_end, type });
      }
